...
/** Reemplaza `var(--nombre)` (con o sin fallback) por el valor computado del `:root`
 *  actual, en cualquier texto (XML de un SVG o CSS de una hoja de estilos). */
function resolverVariablesCss(texto: string): string {
  const raiz = getComputedStyle(document.documentElement);
  return texto.replace(/var\((--[a-zA-Z0-9-]+)(?:,[^)]*)?\)/g, (match, nombre: string) => {
